...
> **对应版本**：syncthing-rust v0.2.7 (commit a665f8f)
> **Windows 侧 Device ID**：`4FXSKHU-G6OXMMR-ECXF2YK-QHWOROO-4K7VA4B-DHRIWWR-IROUJY5-CMHXYAB`
...
**目标**：验证 v0.2.7 在持续运行 72 小时内的连接稳定性和内存泄漏。
...
⚠️ **注意**：v0.2.7 部分 API 端点存在空引用异常（D-4），若返回 500，请使用 `netstat` 和文件系统检查代替。
