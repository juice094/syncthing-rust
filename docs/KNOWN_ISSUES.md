...
> **最后更新**：2026-07-21（§22 数据安全事故三层防御落地；§21 版本向量计数器 ID 修复）
...
| 单元测试覆盖 | 433/433 通过（2026-07-21 实测；含 §20/§21/§22 修复回归测试） |
...
## §22. 云端陈旧索引致本地课程目录批量删除（严重 — 数据安全事故，**已修复 ✅** commit `0be7f19`）

**症状**（2026-07-20 本地 09:58 与 16:23 两波）：puller 应用云端（IYGOGGD）索引中的删除标记，共删除 114 个路径——`obsidian-notes`（10-Courses）60 个（信息检索与搜索引擎/*、农业大数据应用/* 等课程目录）+ `default` 54 个。`versioning: null`，无 syncthing 侧备份。

**根因链**：
1. 云端索引已腐败（大量 BEP error code 2 / NO_SUCH_FILE——通告了无实体的文件）；
2. §21 版本向量计数器硬编码使云端删除标记被误判为线性支配（修复于 `8225c0f`）；
3. 删除以 IndexUpdate 增量批次进入，v3.0.4 的 50% 全量索引阈值（§15）无法覆盖；
4. puller 硬删无回收站，删除动作仅 debug 级日志，事故后定位成本高。

**修复**（三层纵深防御）：
- **删除配额**：单次 pull 删除数 > `max(20, 本地文件数×25%)` 时拒绝整个删除批次（`PULL_DELETE_QUOTA_MIN/RATIO`）。
- **软删除**：puller 远程驱动删除一律隔离到 `<folder>/.sttrash/<日期>/`（7 天保留，pull 时清理；scanner 默认忽略防循环）。
- **删除审计**：index_handler 对每条接受的远程删除输出 info 级日志（文件、来源设备、双端版本向量）。
- 另加 **stale index 告警**：单次 pull 中 error code 2 超过 10 次即警告对端索引可能陈旧。

**恢复路径**：手机（HONOR）事故期间离线，未接收删除，为最佳恢复源；云端实体文件可能仍在。

**残留事项**：回收站无自动恢复 UI（人工/SOP 恢复）；建议重要文件夹启用 versioning。
