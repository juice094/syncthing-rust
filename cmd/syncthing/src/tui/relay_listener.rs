        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match run_relay_listener(
                    ...
                    &relay_device_name,
                    &relay_public_addrs,
                    &relay_global_discovery,
                )
                .await
                {
                    Ok(()) => warn!(
                        "Relay listener for {} exited, reconnecting in {:?}",
                        relay_url, backoff
                    ),
                    Err(e) => warn!(
                        "Relay listener for {} error: {}, reconnecting in {:?}",
                        relay_url, e, backoff
                    ),
                }
                tokio::select! {
...
async fn run_relay_listener(
    ...
    let mut client =
        syncthing_net::relay::RelayProtocolClient::connect(relay_addr, &rustls_config).await?;
    client.join_relay().await?;
    info!("Relay listener joined {}", relay_addr);

    // Register relay URL with Global Discovery so peers can find us
    {
        let mut addrs = public_addrs.lock().await;
        if !addrs.iter().any(|a| a == relay_url) {
            addrs.push(relay_url.to_string());
            info!("Added relay URL to discovery announce list: {}", relay_url);
        }
    }
    if let Some(gd) = global_discovery.as_ref() {
        gd.trigger_reannounce();
        info!("Triggered Global Discovery re-announce after relay join");
    }
    ...
    loop {
        ...
        let session_addr =
            syncthing_net::relay::dial::resolve_session_addr(relay_addr, &invitation)?;
        let session_stream =
            syncthing_net::relay::join_session(session_addr, &invitation.key).await?;
        ...
        if invitation.server_socket {
            ...
            info!(
                "Relay incoming (server) connection registered for {}",
                peer_device
            );
        } else {
            let (mut tls_stream, peer_device) =
            ...
            handle.register_connection(peer_device, conn).await?;
            info!(
                "Relay incoming (client) connection registered for {}",
                peer_device
            );
        }
    }
