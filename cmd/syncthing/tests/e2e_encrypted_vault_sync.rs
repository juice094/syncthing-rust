...
#[tokio::test]
#[serial_test::serial]
async fn test_encrypted_vault_syncs_over_syncthing() {
    ...
    let node_a = TestNode::new("crypto-sync-a").await.expect("create node a");
    let node_b = TestNode::new("crypto-sync-b").await.expect("create node b");
