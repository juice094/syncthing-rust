...
async fn test_two_node_single_file_sync() {
    ...
    while start.elapsed() < Duration::from_secs(90) {
        ...
        // WORKAROUND for §1 ClusterConfig race: if sync stalls, force reconnect.
        // reconnect_device disconnects + clears pending + dials, ensuring
        // on_connected is re-triggered even when a stale connection exists.
        if start.elapsed() > Duration::from_secs(30) && !retried {
            tracing::warn!("Sync stalled, retrying connections (§1 workaround)");
            let _ = node_a.reconnect_to(&node_b).await;
            let _ = node_b.reconnect_to(&node_a).await;
            let _ = node_a
